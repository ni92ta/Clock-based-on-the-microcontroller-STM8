...
unsigned char SB4_push = 0;//Флаг нажатия кнопки переключения часы/минуты будильников
...
segment_clear (5);//очистка сегмента
				LCD_SetPos(14,0);
				if (tuk == 1){
				sendbyte(0b11101101,1);//вывод символа "колокольчик"
				LCD_SetPos(15,0);
				sendbyte(alarm_number,1);//вывод номера будильника
											}
					else{
					sendbyte(0b00100000,1);//очистка сегмента
					LCD_SetPos(15,0);
...
while (CLK->SWCR & (1<<1) == 1){}// Ждем готовности переключения
	...
	//--------------------------------------------------------
	delay_ms(1);
	DS1307init();//Инициализация часовой МС
	LCD_Init();//Инициализация ЖКИ
	delay_ms(1);
	sendbyte(0b01000000,0);//Устанавливаем адресс CGRAM 
  sets_CGRAM (str01);
  sets_CGRAM (str02);
  sets_CGRAM (str03);
  sets_CGRAM (str04);
